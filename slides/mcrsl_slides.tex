...
\subsection{Results}
\label{sec:main}
Table \ref{tab:breakdown} presents the cumulative results of the proposed method, building from the channel-gated baseline to the full pipeline.
\begin{table}[htbp]
\caption{Results of the Proposed Method. Rows 1--4 are cumulative; SAM is listed separately because it does not stack.}
\label{tab:breakdown}
\centering
\scriptsize
...
\hline
\textbf{\#} & \textbf{Step} & \textbf{Quality Used} & \textbf{Bal.\ acc.} & \textbf{Macro-F1} \\
\hline
1 & Channel-gated baseline & No & 0.783 & 0.757 \\
2 & 1 $+$ \textbf{Quality-Adaptive} loss & \textbf{Yes} & 0.820 & 0.778 \\
3 & 2 $+$ test-time augmentation & \textbf{Yes} & 0.833 & 0.783 \\
4 & 3 $+$ multi-image averaging (Proposed) & \textbf{Yes} & \textbf{0.840} & \textbf{0.810} \\
\hline
5 & 3 $+$ SAM optimizer & \textbf{Yes} & 0.801 & 0.761 \\
\hline
\end{tabular}
\end{table}
...
\textbf{Causal validation via a shuffled-quality control:}
\label{sec:shuffle}
...
Table \ref{tab:ablation} compares late fusion and channel-gated fusion of image and metadata features. Channel-gated fusion outperforms late fusion on every metric: accuracy (0.833 vs.\ 0.831), balanced accuracy (0.783 vs.\ 0.768), macro-F1 (0.757 vs.\ 0.743), sensitivity (0.672 vs.\ 0.644), specificity (0.895 vs.\ 0.891), and AUROC (0.881 vs.\ 0.851). We adopt channel-gated fusion as the fusion mechanism for the remainder of this work: beyond its consistent advantage here, it is the only one of the two mechanisms whose per-channel gate provides a natural injection point for the quality signal, on which the quality-adaptive loss reported above is built.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[htbp]
\caption{Ablation Results of the Proposed Fusion Mechanism with both modalities}
\label{tab:ablation}
\centering
\scriptsize
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{Late fusion} & \textbf{Channel-gated} \\
\hline
Accuracy & 0.831 $\pm$ 0.052 & \textbf{0.833 $\pm$ 0.070} \\
Balanced acc. & 0.768 $\pm$ 0.090 & \textbf{0.783 $\pm$ 0.085} \\
Macro F1 & 0.743 $\pm$ 0.051 & \textbf{0.757 $\pm$ 0.073} \\
Sensitivity & 0.644 $\pm$ 0.191 & \textbf{0.672 $\pm$ 0.159} \\
Specificity & 0.891 $\pm$ 0.021 & \textbf{0.895 $\pm$ 0.048} \\
AUROC & 0.851 $\pm$ 0.080 & \textbf{0.881 $\pm$ 0.065} \\
\hline
\end{tabular}
\end{table}
...
MCR-SL has had no prior published benchmark, so the results reported here are the first on this dataset. This is not framed as a state-of-the-art claim, since there is no prior number to have surpassed; the loss mechanism and its validation remain the principal contribution, Table~\ref{tab:literature} therefore compares the configurations evaluated in this work against one another, grouped by the component each one varies.

\begin{table}[htbp]
\caption{Configurations evaluated on MCR-SL; 234 lesions, binary task, mean over five folds.}
\label{tab:literature}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{|c|l|c|c|c|}
\hline
\textbf{\#} & \textbf{Configuration} & \textbf{Built on} & \textbf{Bal.acc} & \textbf{Macro-F1} \\
\hline
\multicolumn{5}{|l|}{\emph{Architecture} --- fusion mechanism, standard weighted BCE} \\
\hline
1 & Image-only encoder            & ---  & 0.784 & 0.733 \\
2 & Late fusion (concatenation)   & ---  & 0.768 & 0.743 \\
3 & Channel-gated fusion          & ---  & 0.783 & 0.757 \\
\hline
\multicolumn{5}{|l|}{\emph{Training objective} --- how the quality rating enters the loss} \\
\hline
4 & $+$ quality-auxiliary head       & 3 & 0.740 & 0.715 \\
5 & $+$ \emph{trust} reweighting     & 3 & 0.788 & 0.721 \\
6 & $+$ \emph{Quality-Adaptive} loss & 3 & \textbf{0.820} & \textbf{0.778} \\
\hline
\multicolumn{5}{|l|}{\emph{Inference} --- evaluation-time only, no retraining} \\
\hline
7 & $+$ test-time augmentation    & 6 & 0.833 & 0.783 \\
8 & $+$ multi-image averaging     & 7 & \textbf{0.840} & \textbf{0.810} \\
\hline
\end{tabular}%
}
\end{table}
For external context, the closest binary-task result in the literature is Uliana and Krohling's \cite{diffmic} 0.836 balanced accuracy on PAD-UFES-20 (diffusion classifier, 2298 images, five-fold cross-validation). The 0.004-point gap to this paper's best configuration is far smaller than the fold-to-fold standard deviation, so the two are statistically indistinguishable, and the datasets differ substantially in unit (lesions vs.\ images) and class balance (17.9\% vs.\ 47.5\% malignant). It is therefore discussed here rather than tabulated alongside our own configurations, since it is not a like-for-like comparator.
...
stacked combinations. Table~\ref{tab:sweep} reports them alongside the proposed
configurations. Sharpness-aware minimisation with test-time augmentation is the best of the
non-quality-aware configurations at 0.822; the contrastive term and the alternative
...
\begin{table}[htbp]
\caption{Ablation sweep grouped by the component varied; $\dagger$ marks the proposed quality-adaptive loss.}
\label{tab:sweep}
\centering
\scriptsize
\setlength{\tabcolsep}{2pt}
...
\hline
\multicolumn{6}{|l|}{\emph{Training objective} --- loss function} \\
\hline
$\dagger$ Quality-adaptive loss & 0.845 & 0.820 & 0.764 & 0.875 & 0.906 \\
Focal loss & 0.858 & 0.809 & 0.694 & 0.923 & 0.877 \\
Contrastive auxiliary loss & 0.812 & 0.770 & 0.669 & 0.872 & 0.873 \\
\hline
\multicolumn{6}{|l|}{\emph{Input and optimiser}} \\
\hline
...
\hline
\multicolumn{6}{|l|}{\emph{Inference} --- no retraining} \\
\hline
Multi-image averaging & 0.854 & 0.813 & 0.719 & 0.907 & 0.906 \\
Test-time augmentation (TTA) & 0.841 & 0.788 & 0.672 & 0.904 & 0.897 \\
\hline
\multicolumn{6}{|l|}{\emph{Stacked combinations}} \\
\hline
Preprocess.$+$focal$+$SAM$+$TTA & 0.818 & 0.759 & 0.622 & 0.895 & 0.893 \\
SAM $+$ multi-image & 0.851 & 0.800 & 0.672 & \textbf{0.928} & 0.917 \\
SAM $+$ TTA $+$ multi-image & 0.856 & 0.810 & 0.694 & 0.925 & 0.915 \\
SAM $+$ TTA & 0.861 & 0.822 & 0.719 & 0.925 & 0.908 \\
$\dagger$ Quality-adaptive $+$ TTA & 0.849 & 0.833 & \textbf{0.808} & 0.858 & 0.911 \\
$\dagger$ Quality-adaptive $+$ TTA $+$ multi-image & \textbf{0.874} & \textbf{0.840} & 0.783 & 0.896 & \textbf{0.918} \\
\hline
\end{tabular}
\end{table}
