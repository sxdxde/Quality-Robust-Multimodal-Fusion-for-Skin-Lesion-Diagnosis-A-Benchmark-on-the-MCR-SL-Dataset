...
\label{sec:arch}
Fig.~\ref{fig:arch} shows the pipeline left to right, and this subsection follows it in that order. On the upper path, an ImageNet-pretrained EfficientNet-B0 \cite{efficientnet,imagenet} encodes the dermoscopic image, returning both a 1280-dimensional pooled vector and the final convolutional feature map. On the lower path, a metadata encoder embeds seventeen categorical fields (twelve dimensions each, with a reserved index for missing values, which are never imputed) and four numeric fields (z-scored on training-fold statistics only, each paired with a missingness indicator), and passes their 212-dimensional concatenation through a two-layer perceptron to a 128-dimensional vector.
...
\begin{tikzpicture}[
  box/.style={draw, rounded corners=2pt, align=center, minimum height=1.0cm, minimum width=2.6cm, font=\small, fill=blue!6},
  gbox/.style={draw, rounded corners=2pt, align=center, minimum height=1.0cm, minimum width=2.6cm, font=\small, fill=orange!14},
  hbox/.style={draw, rounded corners=2pt, align=center, minimum height=0.9cm, minimum width=2.3cm, font=\small, fill=green!10},
  lbox/.style={draw, rounded corners=2pt, align=center, minimum height=1.0cm, minimum width=3.0cm, font=\small, fill=red!9, thick},
  qbox/.style={draw, rounded corners=2pt, align=center, minimum height=0.9cm, minimum width=2.8cm, font=\small, fill=yellow!22},
  opn/.style={draw, circle, font=\normalsize, inner sep=2pt},
  arr/.style={-Stealth, thick}
]
% ---------- image path (top), strictly left to right ----------
\node[box] (img)  at (0,1.5)  {Dermoscopic image\\$3{\times}224{\times}224$};
\node[box] (enc)  at (3.9,1.5){EfficientNet-B0\\encoder};
\node[opn] (mul)  at (7.3,1.5){$\otimes$};
\node[box] (pool) at (10.4,1.5){Gated pool\\$+$ Linear/ReLU};
\node[hbox](bin)  at (14.0,1.5){Binary head};
% ---------- metadata path (bottom), strictly left to right ----------
\node[box] (met)  at (0,-1.5) {Patient metadata\\17 categ.\ $+$ 4 num.};
\node[box] (men)  at (3.9,-1.5){Embeddings $+$ MLP};
\node[gbox](gat)  at (7.3,-1.5){$\sigma(\mathrm{Linear})$\\gate $\in[0,1]^{1280}$};
% ---------- quality path -> loss ----------
\node[qbox](qua)  at (10.4,-1.5){Expert quality $q_i$};
\node[lbox](los)  at (14.0,-1.5){$\mathcal{L}_i=w^{\mathrm{cls}}_i w^{\mathrm{qual}}_i\!\times\!$BCE};
\draw[arr] (img) -- (enc);
\draw[arr] (enc) -- node[font=\scriptsize,above]{$1280{\times}7{\times}7$} (mul);
\draw[arr] (mul) -- (pool);
\draw[arr] (pool) -- node[font=\scriptsize,above]{256-d} (bin);
\draw[arr] (met) -- (men);
\draw[arr] (men) -- node[font=\scriptsize,above]{128-d} (gat);
\draw[arr] (gat) -- node[font=\scriptsize,right]{channel gate} (mul);
\draw[arr] (bin) -- (los);
\draw[arr,dashed] (qua) -- node[font=\scriptsize,above]{$w^{\mathrm{qual}}_i{=}1.5{-}\tfrac{q_i-1}{9}$} (los);
\end{tikzpicture}}
\caption{Channel-gated fusion with quality-adaptive loss reweighting, drawn left to right. The image path (top) and the metadata path (bottom) meet at the channel gate $\otimes$, where a metadata-derived sigmoid gate rescales the convolutional feature map channel by channel before pooling. The paper's contribution is the dashed path: each lesion's expert quality rating $q_i$ becomes a fixed scalar coefficient $w^{\mathrm{qual}}_i$ on that sample's binary loss. It is a precomputed constant, not a learned parameter, and is never differentiated.}
\label{fig:arch}
\end{figure*}
...
\begin{table}[htbp]
\caption{Core ablation matrix, mean $\pm$ std over five folds. Bold marks the best value in each row}
\label{tab:ablation}
\centering
\scriptsize
