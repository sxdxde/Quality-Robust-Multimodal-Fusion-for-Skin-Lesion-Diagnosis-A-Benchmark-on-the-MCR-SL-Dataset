...
\textbf{Accuracy} is the proportion of all lesions classified correctly. On its own it is a weak summary here, because predicting every lesion non-malignant would already score above $80\%$ at this class balance.
...
\textbf{Specificity} is the proportion of non-malignant lesions correctly cleared. Low specificity means unnecessary biopsies and avoidable patient anxiety, so it constrains how far sensitivity can be bought.
...
\textbf{AUROC} is the probability that a randomly drawn malignant lesion receives a higher predicted probability than a randomly drawn non-malignant one. It is threshold-independent, so it measures ranking quality separately from where the decision boundary happens to sit.
...
\begin{tabular}{|l|c|c|c|}
\hline
\multicolumn{4}{|l|}{\textbf{Accuracy by expert quality tercile}} \\
\hline
\textbf{Tercile} & \textbf{$N$} & \textbf{Baseline} & \textbf{\emph{hard\_mining}} \\
\hline
Low & 85 & 0.776 & 0.776 \\
Mid & 93 & 0.914 & 0.903 \\
High & 53 & 0.868 & 0.868 \\
\hline
Malignant sensitivity, low tercile & 23 & 0.667 & \textbf{0.810} \\
\hline
\multicolumn{4}{|l|}{\textbf{Certainty vs.\ error (Spearman $\rho$)}} \\
\hline
\textbf{Stratum} & \textbf{$N$} & \textbf{$\rho$} & \textbf{$p$} \\
\hline
Pooled & 231 & $-0.175$ & 0.008 \\
Non-malignant only & 189 & $-0.127$ & 0.083 \\
Malignant only & 42 & $-0.093$ & 0.559 \\
\hline
\multicolumn{4}{|l|}{\textbf{Ground-truth source and intra-subject consistency}} \\
\hline
\textbf{Group} & \textbf{$N$} & \multicolumn{2}{c|}{\textbf{Accuracy}} \\
\hline
Histopathology-confirmed & 28 & \multicolumn{2}{c|}{0.750} \\
Panel-consensus only & 203 & \multicolumn{2}{c|}{0.867} \\
Consistent subjects (null 29.9) & 55 & \multicolumn{2}{c|}{27, $p=0.954$} \\
\hline
\end{tabular}
\end{table}
...
\textbf{Reversed findings.} Three apparently positive results have not survived scrutiny and are reported rather than omitted. \emph{Trust}'s narrower quality-tercile gap was undercut by the shuffled control narrowing it further, as discussed above. The pooled certainty-error correlation dissolves once stratified by class, having reflected class composition rather than an independent relationship. And a proposed change to train on all images per lesion proved on verification to describe what the pipeline already did. At this sample size an uncontrolled first look repeatedly produces positives that controls dissolve, and reporting that pattern is part of the contribution.
...
\textbf{Qualitative analysis.} Gradients have been taken with respect to the feature map \emph{after} the metadata gate, the representation the decision is computed from, with the metadata branch held fixed. Fig.~\ref{fig:hardsamples} compares the baseline and reweighted models on the two lowest-quality malignant lesions the loss has flipped from a miss to a correct call. In the first, a basal cell carcinoma rated 5.7, baseline attention is spread diffusely and the lesion is confidently missed; under reweighting attention contracts onto a single focus and the call becomes correct. In the second, a melanoma rated 4.3, baseline attention falls on the image edge away from the pigmented region; under reweighting it moves onto the lesion. Both lie in the lowest quality tercile, where reweighting assigns its largest weights. The maps are $7\times7$ upsampled and indicate approximate attention only. These are illustrative rather than representative: across the 231 lesions the reweighting fixes six malignant lesions and breaks two, a net gain of four, which is what a $+0.092$ sensitivity amounts to at this scale.
