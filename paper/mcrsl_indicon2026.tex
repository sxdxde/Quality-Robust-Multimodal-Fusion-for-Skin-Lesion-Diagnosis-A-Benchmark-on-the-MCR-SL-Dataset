...
These are illustrative rather than representative: across the 231 lesions the reweighting has changed 13 predictions from incorrect to correct and 14 the other way, but among malignant lesions it has fixed six and broken two, and that net gain of four is what a $+0.092$ sensitivity amounts to at this scale.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/hard_samples_gradcam_compare_1col.pdf}
\caption{Grad-CAM on the post-gate feature map for the two lowest-quality malignant lesions that quality-adaptive reweighting has flipped from a miss to a correct call. $q$ is the expert quality rating, $P$ is $P(\mathrm{malignant})$. In (a) diffuse baseline attention contracts to a single focus; in (b) baseline attention lies off the lesion and moves onto it. Maps are $7\times7$ upsampled, indicating approximate attention only.}
