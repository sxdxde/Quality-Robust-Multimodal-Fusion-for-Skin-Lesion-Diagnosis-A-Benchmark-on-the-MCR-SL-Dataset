...
\node[box] (img) at (0,7.4) {Dermoscopic image};
\node[box] (meta) at (5.2,7.4) {Patient metadata\\(22 fields)};

\node[box] (enc) at (0,5.8) {EfficientNet-B0\\encoder};
\node[box] (menc) at (5.2,5.8) {Categorical embed.\\+ numeric MLP};

\node[gbox] (gate) at (5.2,4.2) {Linear + sigmoid\\channel gate};

\node[opnode] (mult) at (2.6,2.6) {$\otimes$};
\node[box] (pool) at (2.6,1.0) {Gated global pool};

\node[hbox] (bin) at (0,-0.8) {Binary head\\(malignant)};
\node[hbox] (aux) at (2.6,-0.8) {Aux.\ head\\(9-class)};
\node[hbox] (qual) at (5.2,-0.8) {Quality head\\(optional)};

\draw[arr] (img) -- (enc);
\draw[arr] (meta) -- (menc);
\draw[arr] (menc) -- (gate);
\draw[arr] (enc.south) -- ++(0,-1.0) -| (mult.north);
\draw[arr] (gate.west) -| (mult.north);
\draw[arr] (mult) -- (pool);
\draw[arr] (pool.south) -- ++(0,-0.55) -| (bin.north);
\draw[arr] (pool.south) -- (aux.north);
\draw[arr] (pool.south) -- ++(0,-0.55) -| (qual.north);
\end{tikzpicture}
\caption{Channel-gated fusion architecture. The metadata vector is projected to a per-channel sigmoid gate ($\otimes$: elementwise multiplication) that reweights the image encoder's convolutional feature map before pooling and classification.}
\label{fig:arch}
\end{figure}
